...
def upgma(distances: np.ndarray) -> Tree:
    ...
    *unweighted pair group method with arithmetic mean* (UPGMA).
    ...
    cluster_size_v: List[int] = []
    for i in range(distances.shape[0]):
        cluster_size_v.append(1)
    ...
    while True:
        ...
        for k in range(distances_v.shape[0]):
            if not is_clustered_v[k] and k != i_min:
                mean = (
                    (
                        float(distances_v[i_min, k]) * float(cluster_size_v[i_min])
                        + float(distances_v[j_min, k]) * float(cluster_size_v[j_min])
                    ) / float(cluster_size_v[i_min] + cluster_size_v[j_min])
                )
